...
namespace DAL
{
    public class EntityContext
    {
        ...
        public void GetDataFromFile()
        {
            ...
            Entities = (List<Person>)dataProvider.Deserialize(filePathFULL);
        }
        ...
        {
            if (File.Exists(filePathFULL)) ClearFile();

            dataProvider.Serialize(Entities, filePathFULL);        
        }
        private void ClearFile()
        {
            FileStream fs = new FileStream(filePathFULL, FileMode.Truncate, FileAccess.Write);
            fs.Close();
        }
    }
}